				...
				<table class="table table-bordered table-hover">
					<thead>
						<tr>
							...
							<th width="60px">Tên tour du lịch</th>
							<th>Giá tour</th>
							<th width="70px">Nội dung</th>
							<th>Hình ảnh</th>
							<th>Ngày cập nhật</th>
							<th width="50px"></th>
							<th width="50px"></th>
...
$sql= "SELECT * FROM dulich";
$dulichList = executeResult($sql);

$index = 1;
foreach ($dulichList as $item) {
		echo '<tr>
				<td>'.($index++).'</td>
				<td>'.$item['title'].'</td>
				<td>'.$item['price'].'</td>
		...
		echo'<td>'.$item['updated_at'].'</td>
			...
			<td>
				<button class="btn btn-danger" onclick="deleteDulich('.$item['id'].')">Xoá</button>
			</td>
		</tr>';
}
?>
