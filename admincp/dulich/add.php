...
if (!empty($_POST)) {
	...
	if (!empty($title)) {
		...
		if ($id == '') {
			$sql = 'insert into dulich(title, price, content, thumbnail,created_at,updated_at) value("'.$title.'","'.$price.'","'.$content.'","'.$thumbnail.'","'.$created_at.'","'.$updated_at.'")';
		} else {
			$sql = 'update dulich set title = "'.$title.'", price = "'.$price.'", content = "'.$content.'", thumbnail = "'.$thumbnail.'", created_at = "'.$created_at.'", updated_at = "'.$updated_at.'" where id = '.$id;
		}
...
if (isset($_GET['id'])) {
	$id       = $_GET['id'];
	$sql      = 'select * from dulich where id = '.$id;
	$dulich = executeSingleResult($sql);
	if ($dulich != null) {
		$title = $dulich['title'];
		$content = $dulich['content'];
		$price = $dulich['price'];
		$thumbnail = $dulich['thumbnail'];
	}
}
?>
<!DOCTYPE html>
<html>
<head>
	<title>Thêm/Sửa Danh Mục Sách</title>
...
<body>
	<ul class="nav nav-tabs">
	  ...
	  <li class="nav-item">
	    <a class="nav-link active" href="../dulich/index.php">Quản Lý Tour Du Lịch</a>
	  </li>
	  <li class="nav-item">
	...
	<div class="container">
		<div class="panel panel-primary">
			<div class="panel-heading">
				<h2 class="text-center">Thêm/Sửa Danh Mục Sách</h2>
			</div>
