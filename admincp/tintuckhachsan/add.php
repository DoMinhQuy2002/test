...
if (!empty($_POST)) {
	...
	if (!empty($title)) {
		...
		if ($id == '') {
			$sql = 'insert into tintuckhachsan(title, content, thumbnail,created_at,updated_at) value("'.$title.'","'.$content.'","'.$thumbnail.'","'.$created_at.'","'.$updated_at.'")';
		} else {
			$sql = 'update tintuckhachsan set title = "'.$title.'", content = "'.$content.'", thumbnail = "'.$thumbnail.'", created_at = "'.$created_at.'", updated_at = "'.$updated_at.'" where id = '.$id;
		}
...
if (isset($_GET['id'])) {
	$id       = $_GET['id'];
	$sql      = 'select * from tintuckhachsan where id = '.$id;
	$tintuckhachsan = executeSingleResult($sql);
	if ($tintuckhachsan != null) {
		$title = $tintuckhachsan['title'];
		$content = $tintuckhachsan['content'];
		$thumbnail = $tintuckhachsan['thumbnail'];
	}
}
?>
<!DOCTYPE html>
<html>
<head>
	<title>Thêm/Sửa Danh Mục Tin tức Khách Sạn</title>
...
<body>
	<ul class="nav nav-tabs">
	  ...
	  <li class="nav-item">
	    <a class="nav-link active" href="../tintuckhachsan/index.php">Quản Lý Tin tức Khách Sạn</a>
	  </li>
	  <li class="nav-item">
	...
	<div class="container">
		<div class="panel panel-primary">
			<div class="panel-heading">
				<h2 class="text-center">Thêm/Sửa Danh Mục Tin tức Khách Sạn</h2>
			</div>
